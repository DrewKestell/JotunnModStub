...
public class LiquidVolume : MonoBehaviour
{
	...
	private void Save()
	{
		if (!(m_nview == null) && m_nview.IsValid() && m_nview.IsOwner())
		{
			...
			m_nview.GetZDO().Set("LiquidData", array);
	...
	{
		if (!(m_nview == null) && m_nview.IsValid() && m_nview.GetZDO().m_dataRevision != m_lastDataRevision)
		{
			Load();
		}
	}
	...
	private bool HaveSavedData()
	{
		...
		return m_nview.GetZDO().GetByteArray("LiquidData") != null;
	}
	...
	private void Load()
	{
		m_lastDataRevision = m_nview.GetZDO().m_dataRevision;
		m_needsSaving = false;
		byte[] byteArray = m_nview.GetZDO().GetByteArray("LiquidData");
		if (byteArray == null)
		{
			return;
		}
	...
	private void SmoothNormals(List<Vector3> normals, float yScale)
	{
		...
		for (int i = 1; i < num - 1; i++)
		{
			for (int j = 1; j < num - 1; j++)
			{
				...
				vector2.y *= yScale;
				vector3.y *= yScale;
				vector = (normals[i * num + j] = (vector + vector2 + vector3).normalized);
			}
		}
		for (int k = 1; k < num - 1; k++)
		{
			for (int l = 1; l < num - 1; l++)
			{
				...
				vector5.y *= yScale;
				vector6.y *= yScale;
				vector4 = (normals[k * num + l] = (vector4 + vector5 + vector6).normalized);
			}
		}
	}
