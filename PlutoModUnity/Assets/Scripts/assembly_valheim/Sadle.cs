...
public class Sadle : MonoBehaviour, Interactable, Hoverable, IDoodadController
{
	...
	{
		if (m_character.IsSwiming() && !m_character.IsOnGround() && !HaveStamina())
		{
	...
	public string GetHoverText()
	{
		...
		return string.Concat(Localization.instance.Localize(m_hoverText) + Localization.instance.Localize("\n[<color=yellow><b>$KEY_Use</b></color>] $piece_use"), Localization.instance.Localize("\n[<color=yellow><b>$KEY_AltPlace + $KEY_Use</b></color>] $hud_saddle_remove"));
	}
	...
	public bool HaveValidUser()
	{
		...
		GameObject gameObject = ZNetScene.instance.FindInstance(user);
		if (gameObject == null)
		{
			return false;
		}
		return Vector3.Distance(gameObject.transform.position, base.transform.position) < m_maxUseRange;
	}
	...
	{
		return m_nview.GetZDO().GetFloat("stamina", GetMaxStamina());
	}

	private void SetStamina(float stamina)
	{
		m_nview.GetZDO().Set("stamina", stamina);
	}
	...
	private void UpdateStamina(float dt)
	{
		...
		if (m_staminaRegenTimer > 0f || m_character.InAttack() || m_character.IsSwiming())
		{
			return;
		}
		float stamina = GetStamina();
