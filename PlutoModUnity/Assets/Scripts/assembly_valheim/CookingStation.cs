...
public class CookingStation : MonoBehaviour, Interactable, Hoverable
{
	...
	private void Awake()
	{
		...
		if (m_nview.GetZDO() != null)
		{
			...
			m_nview.Register<Vector3>("RemoveDoneItem", RPC_RemoveDoneItem);
	...
	private void DropAllItems()
	{
		...
		void drop(ItemDrop item)
		{
			...
			UnityEngine.Object.Instantiate(item.gameObject, position, rotation);
		}
	}
	...
	private void SpawnItem(string name, int slot, Vector3 userPoint)
	{
		...
		Quaternion rotation = Quaternion.Euler(0f, UnityEngine.Random.Range(0, 360), 0f);
		UnityEngine.Object.Instantiate(itemPrefab, vector, rotation).GetComponent<Rigidbody>().velocity = vector2 * m_spawnForce;
	...
	private bool OnInteract(Humanoid user)
	{
		if (HaveDoneItem())
		{
			m_nview.InvokeRPC("RemoveDoneItem", user.transform.position);
			return true;
		}
