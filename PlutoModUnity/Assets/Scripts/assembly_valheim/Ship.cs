...
public class Ship : MonoBehaviour
{
	...
	private void FixedUpdate()
	{
		...
		float liquidLevel = Floating.GetLiquidLevel(worldCenterOfMass, waveFactor);
		float liquidLevel2 = Floating.GetLiquidLevel(vector3, waveFactor);
		float liquidLevel3 = Floating.GetLiquidLevel(vector4, waveFactor);
		float liquidLevel4 = Floating.GetLiquidLevel(vector, waveFactor);
		float liquidLevel5 = Floating.GetLiquidLevel(vector2, waveFactor);
		float num = (liquidLevel + liquidLevel2 + liquidLevel3 + liquidLevel4 + liquidLevel5) / 5f;
		float num2 = worldCenterOfMass.y - num - m_waterLevelOffset;
		if (!(num2 > m_disableLevel))
		{
			m_body.WakeUp();
			UpdateWaterForce(num2, Time.fixedDeltaTime);
			Vector3 vector5 = new Vector3(vector3.x, liquidLevel2, vector3.z);
			Vector3 vector6 = new Vector3(vector4.x, liquidLevel3, vector4.z);
			Vector3 vector7 = new Vector3(vector.x, liquidLevel4, vector.z);
			Vector3 vector8 = new Vector3(vector2.x, liquidLevel5, vector2.z);
			float fixedDeltaTime = Time.fixedDeltaTime;
