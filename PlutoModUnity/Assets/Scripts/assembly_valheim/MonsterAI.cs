...
public class MonsterAI : BaseAI
{
	...
	private const float m_unreachableTargetRadius = 10f;
	...
	private void UpdateTarget(Humanoid humanoid, float dt, out bool canHearTarget, out bool canSeeTarget)
	{
		...
		if (m_updateTargetTimer <= 0f && !m_character.InAttack())
		{
			...
			if (m_attackPlayerObjects && (m_targetCreature == null || flag3) && !m_character.IsTamed())
			{
	...
	protected override void UpdateAI(float dt)
	{
		...
		if (DespawnInDay() && EnvMan.instance.IsDay() && (m_targetCreature == null || !canSeeTarget))
		{
	...
	public override bool HuntPlayer()
	{
		if (base.HuntPlayer())
		{
			...
			if (DespawnInDay() && EnvMan.instance.IsDay())
			{
				return false;
			}
			return true;
