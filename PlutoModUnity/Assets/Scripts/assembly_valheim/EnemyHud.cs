...
public class EnemyHud : MonoBehaviour
{
	...
	private void LateUpdate()
	{
		...
		{
			if (!(allCharacter == localPlayer) && (!sadle || !(allCharacter == sadle.GetCharacter())) && TestShow(allCharacter))
			{
	...
	private void ShowHud(Character c, bool isMount)
	{
		if (!m_huds.TryGetValue(c, out var value))
		{
			...
			if (isMount)
			{
				...
				value.m_staminaText = value.m_gui.transform.Find("Stamina/StaminaText").GetComponent<Text>();
				value.m_healthText = value.m_gui.transform.Find("Health/HealthText").GetComponent<Text>();
			}
	...
	private void UpdateHuds(Player player, Sadle sadle, float dt)
	{
		...
		foreach (KeyValuePair<Character, HudData> hud in m_huds)
		{
			...
			if (!value.m_character || !TestShow(value.m_character) || value.m_character == character)
			{
				if (character3 == null)
				{
