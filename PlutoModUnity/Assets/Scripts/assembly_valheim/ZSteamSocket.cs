...
public class ZSteamSocket : IDisposable, ISocket
{
	...
	public int GetSendQueueSize()
	{
		...
		if (SteamNetworkingSockets.GetQuickConnectionStatus(m_con, out var pStats))
		{
			num += pStats.m_cbPendingReliable + pStats.m_cbPendingUnreliable + pStats.m_cbSentUnackedReliable;
		}
		return num;
	}
	...
	public int GetCurrentSendRate()
	{
		...
		int num = pStats.m_cbPendingReliable + pStats.m_cbPendingUnreliable + pStats.m_cbSentUnackedReliable;
		foreach (byte[] item in m_sendQueue)
		{
			num += item.Length;
		}
		return num / Mathf.Clamp(pStats.m_nPing, 5, 250) * 1000;
	}
	...
	public void GetConnectionQuality(out float localQuality, out float remoteQuality, out int ping, out float outByteSec, out float inByteSec)
	{
		if (SteamNetworkingSockets.GetQuickConnectionStatus(m_con, out var pStats))
		{
			localQuality = pStats.m_flConnectionQualityLocal;
			remoteQuality = pStats.m_flConnectionQualityRemote;
			ping = pStats.m_nPing;
			outByteSec = pStats.m_flOutBytesPerSec;
			inByteSec = pStats.m_flInBytesPerSec;
		}
		else
		{
			localQuality = 0f;
