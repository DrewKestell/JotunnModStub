...
public class TerrainComp : MonoBehaviour
{
	...
	private static List<TerrainComp> m_instances = new List<TerrainComp>();
	...
	private void Initialize()
	{
		...
		m_modifiedPaint = new bool[m_width * m_width];
		m_paintMask = new Color[m_width * m_width];
	}
	...
	private void Save()
	{
		...
		m_nview.GetZDO().Set("TCData", bytes);
		m_lastDataRevision = m_nview.GetZDO().m_dataRevision;
	}

	private void CheckLoad()
	{
		if (m_nview.GetZDO().m_dataRevision == m_lastDataRevision)
		{
			return;
		}
	...
	{
		m_lastDataRevision = m_nview.GetZDO().m_dataRevision;
		byte[] byteArray = m_nview.GetZDO().GetByteArray("TCData");
		if (byteArray == null)
		{
			return false;
		}
	...
	{
		foreach (TerrainComp instance in m_instances)
		{
			float num = instance.m_size / 2f;
		...
		for (int i = 0; i < num; i++)
		{
			for (int j = 0; j < num; j++)
			{
				...
				if (num3 != 0f || num4 != 0f)
				{
					...
					value = (heights[num2] = Mathf.Clamp(value, num6 - 8f, num6 + 8f));
				}
			}
		}
	...
	private void PaintCleared(Vector3 worldPos, float radius, TerrainModifier.PaintType paintType, bool heightCheck, bool apply)
	{
		...
		for (int i = y - num3; i <= y + num3; i++)
		{
			...
			{
				float num4 = Vector2.Distance(a, new Vector2(j, i));
				if (j >= 0 && i >= 0 && j < m_width && i < m_width && (!heightCheck || !(m_hmap.GetHeight(j, i) > num)))
				{
					...
					m_modifiedPaint[i * m_width + j] = true;
					m_paintMask[i * m_width + j] = color;
				}
			}
		}
	}
	...
	public static bool UpgradeTerrain(Heightmap hmap)
	{
		...
		{
			Console.instance.Print(string.Concat("Skipping terrain at ", hmap.transform.position, " ( another player is currently the owner )"));
			return false;
		}
