...
public class EnvMan : MonoBehaviour
{
	...
	{
		if (instance.IsAfternoon() || instance.IsNight())
		{
			if (!(Player.m_localPlayer == null))
			{
				return DateTime.Now > Player.m_localPlayer.m_wakeupTime + TimeSpan.FromSeconds(m_sleepCooldownSeconds);
			}
			return true;
		}
		return false;
